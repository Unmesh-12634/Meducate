...
import { GestureControls } from '../3d/Viewer';
...
export function SimulationInterface({ onNavigate, onEndSimulation }: SimulationInterfaceProps) {
  ...
  // Derive gesture name from controls since we can't modify the controller to export it directly
  const currentGesture = gestureEnabled ? (gestureControls ? (
    gestureControls.mode === 'dissection' ? 'Dissection (5 Fingers)' :
      gestureControls.mode === 'pathology' ? 'Pathology (Peace Sign)' :
        'Gesture Active'
  ) : 'Show your hand') : '';
  ...
  const getVitalStatus = (vital: string, value: number | string) => {
    // Convert string values to number for comparison (e.g. "120/80")
    // For BP, taking the systolic (first number) is a simple heuristic for this UI state
    let numValue = typeof value === 'number' ? value : parseFloat(value.toString());
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      >
        ...
        <div className="flex items-center gap-4">
          ...
          <motion.button
            ...
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            className={`px-3 py-2 rounded-xl border transition-all flex items-center gap-2 ${gestureGuideOpen
              ? 'bg-[#00A896]/10 text-[#00A896] border-[#00A896]/20'
          ...
          <motion.button
            ...
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            className={`px-3 py-2 rounded-xl border transition-all flex items-center gap-2 ${gestureEnabled
      ...
      <HandGestureController
        ...
        onGestureChange={setGestureControls}
      />
              ...
              {messages.map((message, idx) => (
                ...
                >
                  ...
                  <div className={`flex-1 max-w-[80%] ${message.sender === 'user' ? 'text-right' : 'text-left'}`}>
                    <div
                      className={`inline-block px-4 py-3 rounded-2xl ${message.sender === 'ai'
        ...
        >
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-3">
                    <div
                      className={`w-6 h-6 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all ${isAnswered && isCorrect
          ...
          <AnimatePresence>
            {showExplanation && (
              ...
              >
                <div
                  className={`rounded-xl p-4 border-2 ${selectedAnswer === mcqQuestions[currentMCQ].correctAnswer
