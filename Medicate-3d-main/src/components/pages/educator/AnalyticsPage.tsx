...
export function AnalyticsPage() {
  ...
  // const [courseFilter, setCourseFilter] = useState('all'); // unused
          ...
          <div className="flex gap-2">
            {['week', 'month', 'year'].map((period) => (
              <motion.button
                ...
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className={`px-4 py-2 rounded-xl border-2 transition-all capitalize ${timeFilter === period
            ...
            >
              <div className="flex items-center justify-between mb-4">
                ...
                <motion.div
                  ...
                  className={`text-xs px-2 py-1 rounded ${insight.trend === 'up' ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500'
                    }`}
                >
                  {insight.change}
                </motion.div>
