              ...
              <div className="space-y-2">
                {leaderboard.map((user, index) => (
                  <motion.div
                    ...
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: 0.6 + index * 0.1 }}
                    className={`flex items-center gap-3 p-3 rounded-xl transition-all ${user.isYou ? 'bg-[#00A896]/10 border border-[#00A896]/30' : 'hover:bg-muted'
                      }`}
                  >
                    <div className={`w-6 h-6 rounded-lg flex items-center justify-center text-xs ${user.rank === 1 ? 'bg-[#FFD166] text-white' :
