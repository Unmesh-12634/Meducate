...
import { GeminiService, type Language } from '../../services/GeminiService';
...
export function AskPage({ language = 'en' }: { language?: Language }) {
  const { t, i18n } = useTranslation();
  const tWelcome = () => t('ask.welcome');
  ...
  // Reset chat + stop TTS when language changes
  useEffect(() => {
  ...
  const handleSend = async () => {
    ...
    try {
      const responseText = await GeminiService.generateResponse(input, language);
      ...
      if (!isMuted) {
        ...
        GeminiService.speakResponse(cleanText, language);
      }
    } catch (error) {
  ...
  const handleSuggestionClick = async (text: string) => {
    ...
    setTimeout(async () => {
      ...
      try {
        const responseText = await GeminiService.generateResponse(text, language);
